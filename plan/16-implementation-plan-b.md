...
1. slug整備完了(シート手動入力→再ビルド検証)
2. feature/batch-02 → 03 → 04 → design-preview-ab のマージ
3. 初回デプロイ(現行改善をまずユーザーに届け、B案効果測定のベースラインを取る)
4. **7/10 美辞学ツアーファイナル終了後**(会期中の大規模な見た目変更を避ける)
...
| サブページのB案プレビュー(曲詳細/stats) | 作成中(2026-07-03 自走作業) |
| バッチ5以降 | 着手条件待ち(§1) |
