...
### ⚠️ 要判断: slugの品質(マージ前に決めたい)

130曲中**51曲のslugがハッシュ状**(例: 第六感=`c6976419cc36`、ヒビカセ=`fa5a912a47f8`)で、`/songs/c6976419cc36/` のような共有・SEOに不利なURLになる。slugはスプレッドシート由来のため、選択肢:

1. **(推奨)デプロイ前にシートの主要曲のslug列を人手で整備**(例: `dai-rokkan`)→ 再ビルドで自動反映。未整備曲はハッシュのままでも動作に支障なし。※デプロイ後のslug変更はURL切れになるため、直すなら初回デプロイ前
2. このまま(ハッシュslug)で公開し、後から整備はしない(URL安定優先)
3. 日本語slug(`/songs/第六感/`)へ移行(URLエンコードの見た目問題あり)
